...
var wasmBinaryFile = 'drflac.wasm';
...
function do_fetch(url,start,end,bufferOut){ return Asyncify.handleAsync(async () => { out("waiting for a fetch"); let request = new Request(UTF8ToString(url), { method : 'GET', headers : { 'Range': 'bytes='+start+'-'+end} }); const response = await fetch(request); out("got the fetch response"); const thedata = await response.arrayBuffer(); let dataHeap = new Uint8Array(Module.HEAPU8.buffer, bufferOut, thedata.byteLength); dataHeap.set(new Uint8Array(thedata)); return dataHeap.byteLength; }); }



// STATICTOP = STATIC_BASE + 7904;
...
var asmGlobalArg = {};
var asmLibraryArg = { "__assert_fail": ___assert_fail, "__handle_stack_overflow": ___handle_stack_overflow, "do_fetch": do_fetch, "emscripten_get_sbrk_ptr": _emscripten_get_sbrk_ptr, "emscripten_memcpy_big": _emscripten_memcpy_big, "emscripten_resize_heap": _emscripten_resize_heap, "fd_write": _fd_write, "memory": wasmMemory, "setTempRet0": _setTempRet0, "table": wasmTable };
...
/** @type {function(...*):?} */
var _get_audio = Module["_get_audio"] = createExportWrapper("get_audio");
